...
const customRouter = new CustomRouter(); // Instancia de tu enrutador personalizado

// Vista para registro de usuarios
customRouter.get('/registro', (req, res) => {
  res.render('registration-form');
});

// Ruta para crear un nuevo usuario
customRouter.post('/registro', async (req, res) => {
  try {
    const existingUser = await User.findOne({ email: req.body.email });
    if (existingUser) {
      return res.status(400).json({ message: 'Este correo ya está registrado' });
...
export default customRouter.getRouter(); // Exporta el enrutador personalizado para su uso
